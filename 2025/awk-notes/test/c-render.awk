BEGIN {
    ...
    FS = " "
    basic_types = "char|double|enum|float|int|long|short" \
                  "|signed|struct|union|unsigned|void|const"
    keywords = "static|volatile|typedef|sizeof|auto|break|case|continue" \
...
typing {
    ...
    # 渲染函数参数类型
    $0 = gensub(/\$t{([^}]+)}/, "/BTEX\\\\color[" color["$t"] "]{\\1}/ETEX", "g")
    # 渲染函数参数
