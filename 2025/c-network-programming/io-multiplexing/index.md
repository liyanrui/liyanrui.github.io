...
熟悉 C 语言的人，肯定知道，打开一份文件，需要使用 C 标准库提供的 `fopen` 函数，但是现在我们需要掌握更底层的 `open`，它也能打开文件，而且它是 POSIX 系统（Unix 或 Linux）暴露给用户的接口。像 `open` 这样的函数亦称系统调用，而 `fopen` 则称为库函数，后者可基于前者实现，且功能更为丰富。实际上，套接字 API 函数也是系统调用。
...
我们是用 a.txt 指代一个文件，而对于操作系统而言，它会用 `fd_a` 这样的文件描述符指代一个文件。还记得「[套接字地址](../getaddrinfo/index.html)」吗？人类总是习惯用文字指代某物，而操作系统则习惯使用数字指代某物。两者同出，异名同谓。玄之又玄，众妙之门。
...
`select` 的作用很简单，它能够轮循一组文件——以文件描述符集合表示，从中获得符合需求的子集。不过，理解 `select` 函数的作用，犹如从牛顿力学跃迁到广义相对论，因为它能够让你看到，cpu 的速度是不变的，但时间却可以膨胀或收缩，同时空间在收缩或膨胀，这就是 I/O 多路复用的魅力所在。
...
在「[两朵乌云](../blocking/index.html)」里，我们讨论了服务端在面临许多客户端同时访问时的窘迫，并试图从解除套接字阻塞的途径提高服务端处理并发访问的能力，结果我们失败了。为了稳健地接收和发送数据，似乎服务端要么会陷入 I/O 阻塞，要么会陷入时间阻塞。当服务端陷入时间阻塞的时候，我们看到了第 2 朵乌云，即服务端疯狂地吞噬着 CPU。

我原本以为 `select`函数能够驱散这朵乌云，然而上一节的 files-monitor 试验，却浮现了同样的乌云。
...
普通文件总是可读的，套接字指代的文件未必如此，不过后者只是我的猜想。下一步工作，我们需要基于 select 为网络程序的服务端构建 I/O 多路复用机制，验证它是否能避免疯狂消耗 CPU。

# 总结

乌云犹在，可是你意外学会了不少 Linux 命令。不要忘记，用 `Ctrl` 和 `C` 组合键终止 files-monitor 程序，否则它会继续自我感动而努力运行着。
