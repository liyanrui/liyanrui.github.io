---
title: 这就是网络编程？
abstract: 我在想方设法学会 C socket API 网络编程。
date: 03 月 01 日
...

* <span class="post-date">02 月 26 日</span>[Socket 是什么？](what-is-a-socket/index.html)：一个简单的概念是如何变得难以理解的？
* <span class="post-date">02 月 27 日</span>[网络地址](getaddrinfo/index.html)：将文本形式的 IP 地址和端口转化为数字，是网络编程的核心问题。
* <span class="post-date">03 月 01 日</span>[有一封信，无处可寄](simple-client/index.html)：一个毁灭地球的计划以失败而告终。
* <span class="post-date">03 月 03 日</span>[我是三体人](threebody/index.html)：却并非叶文洁想要找的人。
* <span class="post-date">03 月 05 日</span>[封装](wrapper/index.html)：收纳桌子上那些乱七八糟的电线。
* <span class="post-date">03 月 09 日</span>[两朵乌云](blocking/index.html)：你能同时接听两个电话吗？
* <span class="post-date">03 月 15 日</span>[同步 I/O 多路复用](io-multiplexing/index.html)：拒绝内耗，见机行事。
