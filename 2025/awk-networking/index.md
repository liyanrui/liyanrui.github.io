...
本文是 gawk 网络编程的学习笔记。用 Awk 语言编写网络程序，这样的想法有些癫狂，像是企图用一柄小刀砍伐一棵巨树，何况是对于我这样的人，对网络编程近乎一无所知。对于一些在使用 Awk 语言处理文本方面颇有经验的人，他们甚至未必知道 Awk 语言能够实现网络编程。的确如此，Awk 语言并不支持网络编程，但是 gawk 改变了这个事实。gawk 对 Awk 语言进行了扩展，网络编程便在其中，于 gawk 3.1 版本实现，详见「[Gawkinet: TCP/IP Internetworking with Gawk](https://www.gnu.org/software/gawk/manual/gawkinet/)」。
...
gawk 的网络编程基于双向管道实现，将提供某种网络服务的机器作为协同进程。gawk 以一种特殊文件形式表示网络主机，形式如下：
...
这种特殊文件形式，即可表达服务器，也可表达客户端。例如，将本机作为服务器，使用 IP v4 网络类型和 TCP 协议，以端口 8080 提供某种网络服务，则特殊文件的写法为

```awk
/inet4/tcp/8080/0/0
```

若某个客户端访问该服务器，则该客户端对应的特殊文件写法为

```awk
/inet4/tcp/0/服务器地址(IP 或域名)/8080
```

这种特殊文件的写法，除了网络类型和协议之外的其他部分，无论是服务器程序还是客户端程序，只要自身无需关心的部分，皆写为 0。例如，作为服务器端的程序，它不必关心 `hostname` 和 `remoteport`，因为它不需要访问其他主机，而作为客户端的程序则不必关心 `localport`，因为它不需要向其他主机提供服务。

gawk 将网络主机的初始化过程隐含在上述特殊文件的构建过程中了，从而显著简化了网络程序的编写，但是也牺牲了实现能够应对复杂需求的网络程序的可能性，亦即使用 gawk 通常只能编写简单的网络程序。工业级的网络程序，例如支持网络并发访问和反向代理等功能的服务器程序，需使用其他编程语言在底层的 socket 层面方能实现。

现在，需要将代表网络主机的特殊文件视为运行在本机或其他机器上的程序。使用双向管道可与之连接。若编写服务器端的程序，只需将信息发送到特殊文件，例如

```awk
print "Hello world!" |& "/inet4/tcp/8888/0/0"
```

若编写客户端的程序，只需从特殊文件读取信息：

```awk
"/inet4/tcp/0/服务器地址/8888" |& getline
print $0
```

`getline` 可从服务器端读取一行文本并将其作为当前要处理的一条记录，即 `$0`。


用于表示服务端的特殊文件和用于表示客户端的特殊文件，二者存在一个重叠，即端口。在服务端，端口是本机端口；在客户端，端口是远程端口，二者所指对象是服务器上的那个端口。可以将服务器理解为有许多房间的一栋楼，端口是房间的门牌号。

表示网络主机的特殊文件，网络协议字段是 `inet4` 或 `inet6`，分别表示 IP v4 和 IP v6。可以简写为 `inet`，此时若系统环境使用的是 IP v4 网络，则 `inet` 表示 `inet4`，若系统环境为 IP v6 网络，则 `inet` 即 `inet6`。
...
现在 server.awk 程序会在本机的 `8888` 端口等待客户端的访问。

位于任何网络主机上的客户端程序在访问运行 server.awk 的主机的 `8888` 端口时，可以得到文本「Hello world!」。下面是位于该服务端程序所在主机上的一个客户端程序 client.awk：
...
由于 client.awk 访问的服务器就是本机，故而网络主机的地址是 `localhost`，亦可写为 `127.0.0.1`。运行 client.awk，可在屏幕上打印来自 server.awk 的「Hello world!」，
...
然后 client.awk 和 server.awk 分别自动结束运行。

# 端口查看器

上一节实现的 server.awk 和 client.awk，皆未对表示网络主机的特殊文件进行显式关闭。学究一些，是需要显式关闭的，即
...
现在，可以按照 `netstat` 的术语，将上述用于表示网络主机的特殊文件，称为网络连接，简称连接。
