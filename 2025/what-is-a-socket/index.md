---
title: socket 是什么？
abstract: 计算机网络科学家是如何让一个非常简单的概念变得难以理解的？
date: 02 月 26 日
...
...
网络编程中的 socket，形式上是网络上任何一台计算机的 IP 地址加上某个端口的编号，例如 192.168.0.31:8000，其中冒号之前的部分是 IP 地址，8000 是端口号。socket 的用途是，为网络中一台计算机上的某个进程构造一个用于网络通信的端口地址且该地址具有唯一性。
...
现在要建立一个直觉，所谓网络通信，指的是运行在网络上某台机器或两台不同的机器上的两个进程的通信。假设进程 A 要给进程 B 发送信息，A 需要知道 B 的 socket 以及自己的 socket，类似于你要给一个人写信，信封上需要写上他的收信地址，若你希望他能回信，信封上自然也要写上自己的收信地址。此时，B 并不需要知道 A 或任何一个其他进程要给它发送信息，但 B 需要在任何进程给它发送信息之前创建自己的 socket 并且守着它，等待其他进程传过来的信息。当 B 等到了 A 发来的信息，可以不予理睬，也可以予以回复。若 B 打算回复信息，它需要从 A 发来的信息中找到 A 的 socket，然后向 A 发送信息。同理，A 在向 B 发送信息后，会一直守着自己的 socket 等待着 B 的回复。

进程 A 和 B 一直守在自己的 socket 等待回复，有些愚蠢，但人类有时会宁愿如此愚蠢，例如抱柱的尾生。更聪明一些的办法是，进程可以忙一些别的事情，只是每隔一段时间检查自己的 socket，看有没有已经传过来的信息。最聪明的做法是，建立一个邮局，进程可以完全不用守着 socket，直到邮差的到来。
