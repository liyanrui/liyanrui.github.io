...
```rust
let mut root = Box::into_raw(Box::new(TreeNode::new("Root")));
unsafe {
    (*root).display();
}
```
...
对于 Rust 语言初学者而言，引用、智能指针和原始指针这三者的学习次序，我的建议是原始指针 -> 智能指针 -> 引用，而非多数 Rust 教程建议的引用 -> 智能指针 -> 原始指针。至于这三种指针的用法，我也是推荐先使用原始指针，在遇到难以克服的安全性问题时，再考虑使用智能指针或引用对代码进行重构。不过，对于指针所指内存区域释放问题，C 指针较 Rust 原始指针更为干净利落。
