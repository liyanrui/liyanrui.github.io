...
上述代码构造的两条路径，虽然非常准确，但是过于繁琐。snail 提供了 `经转` 和 `纬转` 宏，可简化直角转弯路段的构造过程。所谓 `经转` 即沿着经向（南北走向）前进时垂直于该走向转弯抵达目的地点，所谓 `经转` 即沿着纬向（东西走向）前进时垂直于该走向转弯抵达目的地点。例如上述两条路径 `甲乙` 和 `乙丙` 的构造语句可简化为
...
路(甲乙, 甲.酉门 纬转 甲.艮位 纬转 乙.子门);
路(乙丙, 乙.午门 经转 丙.坎位 经转 丙.兑位 经转 丙.卯门);
```

![经纬转向][08]

还有两个宏，`经至` 和 `纬至`，有时也能简化路径的构造。例如，从山东淄博向正西前进，至西安所在的经度，该路径可使用 `经至` 构造：
...
路(甲乙, 甲.酉门 经至 乙.离位 经转 乙.卯门);
```

![经至][09]

可以位路径增加标注。snail 会自动选择路径中最长的路段放标注文本。例如
...
```MetaFont
宫(甲, "甲");
宫(乙, "乙"); 定位(乙, 乙.巽位 位于 甲.震位 偏 (东 1cm));
呜呼 甲, 乙;

小弯路(甲甲, 甲.子门 经转 甲.艮位 经转 甲.酉门);
弯路(甲乙, 甲.卯门 -- 甲.离位 经转 乙.乾位 -- 乙.午门) 定位标注("奔赴", "北", .5);
小弯路(乙甲, 乙.酉门 纬转 甲.丑门);
```
...
`小弯路` 较 `弯路` 更接近于折线路径。上述示例也演示了 `定位标注` 的用法 。与 `标注` 相比，`定位标注` 可在自行指定曲线参数对应的点位置放置标注文本。 曲线参数取值范围为 [0, 1]。
...
[b]: ../../figures/metafun/new-snail/b.png
