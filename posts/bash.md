...
在这个世界里，持着光，我注意的第一个东西是「`$`」，它的右面是一个不断明灭的矩形块，顶部是「`~`」。「`$`」称作命令提示符。闪烁的矩形块表示可以在此输入命令。「`~`」是我当前所在的位置——硬盘里的某个目录；实际上 `~` 是 Bash 对我的个人主目录给出的简写，它的全称是「`/home/garfileo`」。倘若我此时在`/tmp` 目录，那么`$` 的顶部应当是「`/tmp`」。
