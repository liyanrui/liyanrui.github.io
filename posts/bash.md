...
图形界面与命令并不冲突。我们教孩子说话，一开始也是以指物的方式，让孩子获得了基本的语言交流能力，在这个基础上再教他们识文断字。图形界面程序为计算机的普及作出了很大贡献，但是在这个基础上，若想让计算机更准确、高效地理解我们的意图，那么便需要学习一门计算机语言了。
...
`mv`、`sed` 和 `bash` 这些命令对应的可执行文件皆位于 `/bin` 目录之内。这些可执行文件的名字，类似于人类语言里的「动词」。文本或文件（包括 stdin 和 stdout），它们存储着程序的输入数据和输出数据，类似于「名词」。有动词，有名词，便可以造句了。例如，
...
可将这条命令翻译为汉语，「写出『echo "Hello world"』，传给 bash！」，也可以翻译成英文，「write "echo 'Hello world'" and pass it to bash!」。
...
很容易去问，为什么会有两种字串？其实应该去问，为什么人类发明的文字里要有两种引号？在与他人的对话中，彼此所说的每句话可以用双引号包围起来。例如：
...
Bash 便会对文本中的 `!` 会有特殊的理解，因为 `!` 是 Bash 命令，想不到吧？

双引号字串与单引号字串各有其用，宽严有度的人通常不会抱怨为什么要有两种引号，为什么要有两种字串。

# 名可名

`PS1` 是一个名字，「`\e[0;32m\w\e[0m\n$ `」是它指代的对象，`$PS1` 是引用这个对象。在 Bash 的世界里，允许我们以这样的方式对数据予以命名，而后以名字指代数据。例如
...
名字，这是我们很熟悉的概念，但是在数学和编程中，它不再叫名字，而是叫「变量」。为数据取一个名字，就是「定义一个变量」，而使用这个名字，叫做「变量解引用」。很寻常的做法，一旦换了名目，就立刻令人觉得莫测高深了起来。

名字之所以被视为变量，肯定不是因为老子说过「名可名，非恒名」。为数据取名字，只是为了方便数据的重复使用。譬如，一旦定义了变量 `hello`，便可以重复使用它：
...
这条命令利用了双引号字串的宽松，实现了在字串中对变量进行解引用，从而起到了言语简约但意义完备的效果。由于双引号字串较为宽松，字串中若出现引号，会引起 Bash 的解读在我们看来是混乱的，因此在上述命令中需要使用 `\` 对字串中的引号进行转义，让 Bash 将其视为字面上的引号，而非字串的引号。倘若去掉转移符 `\`，即
...
在双引号字串中，为避免 Bash 对某些我们期望保持本义的字符产生误解，通常需要用转义符 `\` 让 Bash 放弃这样的尝试。因此，

```console
$ echo "I say: \"$hello\""
```

Bash 会认为 `echo` 的输入数据是「`I say: "$hello"`」。

要注意，在定义变量时，等号两侧不能出现空白字符。例如

```console
$ my_var = "3"
```

Bash 不会认为这条语句是在定义变量 `my_var`，反而会认为 `my_var` 是命令，`=` 和 "3" 是它的输入数据。
