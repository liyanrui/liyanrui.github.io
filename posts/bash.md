...
不过，上述的 `sed` 命令的第一行虽然在末尾出现了续行符，但实际上 Bash 是没有机会得知该续行符的存在。因为 sed 程序所接受的命令文本是拘禁在一对单引号中的，这样的文本中的换行符，Bash 会不予理睬。亦即，上述的 `sed` 命令的第一行末尾的 `\`，实际上并非 Bash 的续行符，它与 `i` 的组合 `i \` 实际上是 sed 程序的一个指令，用于在指定的文本行之前插入一行或多行给定的文本。也是因为对于 sed 程序而言， `\` 有特殊含义，因此使用 `sed` 命令在 foo.tex 的首部添加「`\usemodule[zhfonts]`」时，需要对文本中的 `\` 进行转义，而 sed 程序是将 `\` 作为转义符，因此在 sed 命令中，「`\usemodule[zhfonts]`」必须写成「`\\usemodule[zhfonts]`」。
