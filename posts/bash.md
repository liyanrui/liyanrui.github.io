...
四两拨千斤，并不神奇。你的车若是抛锚了，若想将它运送到修理厂，只需拨打一个特定的电话，不一会的工夫就会出现一辆拖车，它会将你的车运到修理厂。学习计算机语言，也能够在计算机的世界里获得这样的能力。获得该能力并不需要去读计算机科学专业的学位，如同学习开车并不需要你去读车辆工程专业的学位。
...
终端是 Shell 的界面，人通过终端输入命令。Shell 从终端接到命令，然后理解命令，执行命令。

在这个世界里，持着光，我注意的第一个东西是「`$`」，它的右面是一个不断明灭的矩形块，顶部是「`~`」。「`$`」称作命令提示符。闪烁的矩形块表示可以在此输入命令。「`~`」是我当前所在的位置——硬盘里的某个目录；实际上 `~` 是我的个人主目录的简写，它的全称是「`/home/garfileo`」。倘若我此时在`/tmp` 目录，那么`$` 的顶部应当是「`/tmp`」。
...
`\e[0;32m` 和 `\e[0m` 皆为 Bash 世界里的颜色值，前者表示绿色，后者表示无色；当二者之间出现 `\w` 时，所产生的效果是设定 `\w` 为绿色，但是不影响 `\w` 以外的字符。`\w` 表示当前所在的位置。`\n` 表示换行。这种咒语级的代码，足以令很多人望 Bash 而生畏或生厌了。
